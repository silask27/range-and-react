...
def _check_model_runtime() -> RuntimeCheckResult:
    try:
        ...
        return RuntimeCheckResult("predictive_models", "ok", "V5 action + size-model-v2 runtime artifacts and dependencies are ready")
    except Exception as exc:
...
def _check_email_delivery() -> RuntimeCheckResult:
    ...
    return RuntimeCheckResult("email_delivery", "error", "Email provider is set but required credentials are incomplete", required=False)
...
def _check_legal_metadata() -> RuntimeCheckResult:
    ...
    if issues:
        return RuntimeCheckResult("legal_metadata", "warn", "Legal/support metadata needs review: " + "; ".join(issues), required=False)
...
def _check_production_runtime() -> list[RuntimeCheckResult]:
    ...
    if settings.public_status_detailed_checks:
        checks.append(RuntimeCheckResult("production_public_status_detail", "warn", "Public status endpoint exposes detailed runtime checks; disable for public production pages", required=False))
    else:
...
def run_runtime_checks() -> list[RuntimeCheckResult]:
    checks = [_check_database(), _check_model_runtime(), _check_email_delivery(), _check_sentry(), _check_legal_metadata()]
...
def summarize_runtime_checks(checks: list[RuntimeCheckResult]) -> dict[str, Any]:
    ...
    return {"status": "ok" if not errors else "degraded", "error_count": len(errors), "warning_count": len(warnings), "checks": [check.to_dict() for check in checks]}
