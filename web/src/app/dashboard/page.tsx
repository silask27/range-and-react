      ...
      {user ? (
        <div style={{ display: "grid", gap: 28 }}>
          <section className="open-grid-four">
            ...
            {isCoach ? (
              <>
                <HomeLinkCard href="/coach" icon={<ChartIcon />} title="Coach" copy="Pool-wide analytics, assignments, review queue, and reporting for your member roster." extra={<div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><span className="badge badge-success">Pool Range {formatScore(coachSummary?.avg_ranging_score ?? null)}</span><span className="badge badge-primary">Pool Action {formatScore(coachSummary?.avg_response_score ?? null)}</span></div>} />
