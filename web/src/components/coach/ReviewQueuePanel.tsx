...
export default function ReviewQueuePanel({ user, onStatsChange }: { user: AuthUser | null; onStatsChange?: (stats: { memberFlags: number; coachExamples: number; members: number }) => void }) {
  ...
  useEffect(() => {
    onStatsChange?.({ memberFlags: memberRows.length, coachExamples: exampleRows.length, members: memberOptions.length });
  }, [exampleRows.length, memberOptions.length, memberRows.length, onStatsChange]);

  return (
    <div style={{ display: "grid", gap: 24 }}>
      {isLoading ? <div style={panelStyle}>Loading review queue...</div> : null}
...
export function ReviewHeaderStats({ memberFlags, coachExamples, members }: { memberFlags: number; coachExamples: number; members: number }) {
  return (
    <>
      <HeaderStat label="Member Flags" value={memberFlags} tone="coral" />
      <HeaderStat label="Coach Examples" value={coachExamples} tone="green" />
      <HeaderStat label="Members" value={members} tone="neutral" />
    </>
  );
}
...
const headerStatStyle: CSSProperties = { flex: "0 0 174px", width: "100%", maxWidth: 174, minHeight: 86, borderRadius: 18, padding: "13px 15px", border: "1px solid var(--line)", display: "flex", flexDirection: "column", justifyContent: "space-between" };
